...
$checks = @(
  ...
  @{ Name = 'article aside stays compact and reserves room for card shadows'; Ok = $cssText -match '\.article-aside\{[^}]*padding-left:\s*0(?:;|\s|\})[^}]*padding-right:\s*(?:0\.5rem|\.5rem)[^}]*padding-bottom:\s*(?:0\.5rem|\.5rem)' -and $cssText -match '\.article-toc-card,\s*\.series-reading-card\{[^}]*max-width:\s*12rem' },
