...
$checks = @(
  ...
  @{ Name = 'giscus config is defined in hugo params'; Ok = $hugoToml -match '\[params\.giscus\]' -and $hugoToml -match "repo = 'koialkaid/blog'" -and $hugoToml -match "repoId = 'R_kgDOR-EI_A'" -and $hugoToml -match "categoryId = 'DIC_kwDOR-EI_M4C7CtO'" -and $hugoToml -match "inputPosition = 'top'" },
  @{ Name = 'article page uses balanced reading-first two-column shell'; Ok = $cssText -match '\.article-page-shell\{[^}]*grid-template-columns:\s*minmax\(0,\s*52rem\)\s*minmax\(16rem,\s*16rem\)[^}]*gap:\s*2rem' },
  ...
  @{ Name = 'article pages render giscus directly without extra intro block'; Ok = ($postHtml -eq '' -or ($postHtml -match 'giscus\.app/client\.js' -and $postHtml -match 'data-repo=koialkaid/blog' -and $postHtml -match 'data-repo-id=R_kgDOR-EI_A' -and $postHtml -match 'data-category=Announcements' -and $postHtml -match 'data-category-id=DIC_kwDOR-EI_M4C7CtO' -and $postHtml -match 'id=comments')) -and ($noteHtml -eq '' -or ($noteHtml -match 'giscus\.app/client\.js' -and $noteHtml -match 'id=comments')) -and $commentsPartial -notmatch 'section-kicker">Comments<' -and $commentsPartial -notmatch '登录 GitHub 后即可评论' },
  @{ Name = 'article no longer renders a header jump link for comments'; Ok = $singleTemplate -notmatch 'article-comments-jump' -and $singleTemplate -notmatch 'href="#comments"' -and $cssText -notmatch '\.article-comments-jump\{' },
  @{ Name = 'giscus input box is configured above the comment list'; Ok = ($postHtml -eq '' -or $postHtml -match 'data-input-position=top') -and ($noteHtml -eq '' -or $noteHtml -match 'data-input-position=top') },
  @{ Name = 'comment section keeps natural height so editor stays visible'; Ok = $cssText -match '\.article-comments\{[^}]*scroll-margin-top:\s*1\.5rem' -and $cssText -notmatch '\.article-comments-frame\{[^}]*max-height:\s*28rem' },
  ...
  @{ Name = 'article aside stays compact and reserves room for card shadows'; Ok = $cssText -match '\.article-aside\{[^}]*padding-left:\s*0(?:;|\s|\})[^}]*padding-right:\s*(?:0\.5rem|\.5rem)[^}]*padding-bottom:\s*(?:0\.5rem|\.5rem)' -and $cssText -match '\.article-toc-card,\s*\.series-reading-card\{[^}]*max-width:\s*16rem' },
  ...
  @{ Name = 'toc card uses a stronger visible shadow than the base card stack'; Ok = $cssText -match '\.article-toc-card\{[^}]*box-shadow:\s*8px\s+8px(?:\s+0)?\s+rgba\(0,\s*0,\s*0,\s*(?:0\.05|5%)\)' },
