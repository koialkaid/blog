...
$checks = @(
  ...
  @{ Name = 'article header offers a jump-to-comments link'; Ok = $singleTemplate -match 'article-comments-jump' -and $singleTemplate -match 'href="#comments"' -and $cssText -match '\.article-comments-jump\{[^}]*border-bottom:\s*1px solid var\(--line\)[^}]*color:\s*var\(--muted\)' },
